...
from guirpc.amqp.domain import ProxyRequest, ProxyResponse
...
def change_bi_status(bi_uuid: UUID, status_method: str):
    payload = {
        'bi_uuid': str(bi_uuid),
...
class ScaladeWSRpcClient:
    def run_method(self, mth: str, *args, **kwargs):
        rpc_func = getattr(sys.modules[__name__], mth)
        return self.eval_response(
            rpc_func.__call__(*args, **kwargs)
        )

    @staticmethod
    def eval_response(response):
        if response.is_success and response.object['success']:
